  ...
  switch (field) {
    ...
    case 'enrollment':
      ...
      if (!ENROLLMENT_REGEX.test(value.trim()))
        return 'Format must be like S24CSEU0193 (e.g. letter · 2 digits · dept code · U · 4 digits).';
      return '';
    case 'email':
      if (!value.trim()) return 'University email is required.';
      if (!value.toLowerCase().endsWith(EMAIL_DOMAIN))
        return `Must be a @bennett.edu.in address (e.g. you@bennett.edu.in).`;
      if (!/^[^\s@]+@bennett\.edu\.in$/.test(value.toLowerCase()))
        return 'Enter a valid @bennett.edu.in email address.';
      return '';
    case 'password':
...
export default function RegisterPage() {
  ...
  const Field = ({
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      {showError(id) ? (
        <p style={errorStyle}>
          <span style={{ fontSize: 13 }}>⚠</span> {errors[id]}
        </p>
      ) : hint ? (
            ...
            <form onSubmit={handleStep1} noValidate>
              ...
              >
                <input
                  ...
                  className="input"
                  type="text"
                  placeholder="Nakul Thakur"
                  value={form.name}
                  onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  onBlur={() => handleBlur('name')}
                  style={{
                    ...inputBase,
                    borderColor: showError('name') ? '#e53935' : undefined,
                  }}
                />
              </Field>
              ...
              <Field
                ...
                hint="Format: S24CSEU0193"
              >
                <input
                  id="enrollment"
                  className="input"
                  type="text"
                  placeholder="S24CSEU0193"
                  ...
                  style={{
                    ...
                    borderColor: showError('enrollment') ? '#e53935' : undefined,
                  }}
                />
              </Field>
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} noValidate>
              ...
              <Field
                ...
                hint="Must be your @bennett.edu.in address."
              >
                <input
                  id="email"
                  className="input"
                  ...
                  style={{
                    ...inputBase,
                    borderColor: showError('email') ? '#e53935' : undefined,
                  }}
                />
              </Field>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{
                      ...inputBase,
                      paddingRight: 44,
                      borderColor: showError('password') ? '#e53935' : undefined,
                    }}
                    minLength={8}
                  />
                  <button
